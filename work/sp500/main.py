...
DATA_DIR = PROJECT_ROOT / "data" / "sp500"
OUTPUT_DIR = PROJECT_ROOT / "work" / "sp500" / "output"
...
# ---------------------------------------------------------------------
# Forecaster for one CSV (one Close series)
# ---------------------------------------------------------------------
class SP500Forecaster:
    """
    One-step-ahead forecasting of 'Close' using past 'Close' values only
    (univariate autoregressive LSTM).
    """
    ...
    def load_dataframe(self) -> pd.DataFrame:
        ...
        # Expect standard OHLCV columns
        if "Date" not in df.columns or "Close" not in df.columns:
            raise ValueError("CSV must have columns 'Date' and 'Close'.")
    ...
    def preprocess(self):
        ...
        close_vals = df["Close"].values.astype(float).reshape(-1, 1)
        ...
        self.scaler.fit(close_vals[:train_end])

        close_scaled = self.scaler.transform(close_vals).ravel()

        seq_len = self.config.seq_len

        # train
        X_train, y_train = create_sequences(
            close_scaled[:train_end].reshape(-1, 1),
            close_scaled[:train_end],
            seq_len,
        )

        # val (allow overlap)
        X_val, y_val = create_sequences(
            close_scaled[train_end - seq_len : val_end].reshape(-1, 1),
            close_scaled[train_end - seq_len : val_end],
            seq_len,
        )

        # test
        X_test, y_test = create_sequences(
            close_scaled[val_end - seq_len :].reshape(-1, 1),
            close_scaled[val_end - seq_len :],
            seq_len,
        )
...
def plot_saliency_time(
    ...
    - show Close(t) over the window (unscaled)
    ...
    ax1.plot(t_idx, x_unscaled, label="Close", linewidth=2)
...
def extract_local_shapelet(
    ...
    and their corresponding Close values.
    """
    ...
    close_vals = result["close_values"]
    ...
    return {
        ...
        "close_values": close_vals[top_sorted],
...
def run_training_for_csv(csv_path: Path):
    forecaster = SP500Forecaster(csv_path=csv_path)
